...
<div class="top960"><?php echo getadsurl('top960');?></div>
    <div class="vodlist_r box">
        <div class="top">
            <h3><span>评分</span><a>下载排行榜</a></h3>
        </div>
        <ul><?php $gold_desc = ff_mysql_vod('limit:30;order:vod_gold desc,vod_hits desc'); if(is_array($gold_desc)): $i = 0; $__LIST__ = $gold_desc;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$ppvod): $mod = ($i % 2 );++$i;?><li><span><?php echo ($ppvod["vod_gold"]); ?></span><em><?php echo ($i); ?></em><a href="<?php echo ($ppvod["vod_readurl"]); ?>"><?php echo (msubstr($ppvod["vod_name"],0,12)); ?></a></li><?php endforeach; endif; else: echo "" ;endif; ?></ul>        
    </div>   
    <div class="vodlist_l box">
      ...
      <?php else: ?>
       	 <ul>对不起,没有搜索到<<?php echo ($search_wd); echo ($search_actor); echo ($search_director); echo ((isset($search_year) && ($search_year !== ""))?($search_year):''); echo ($search_language); echo ($search_letter); ?>>相关内容!</ul><?php endif; ?>    
    </div>   
    <div class="blank"></div>
   <div class="ft">
    <div class="ft_l">
