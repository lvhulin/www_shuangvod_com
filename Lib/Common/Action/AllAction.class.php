...
class AllAction extends Action{
	...
	* @返回赋值后的arrays 多维数组*/
	public function Lable_Vod_Read($array,$array_play=false){
		...
		$array['vod_readurl'] = ff_data_url('vod',$array['vod_id'],$array['vod_cid'],$array['vod_name'],1,$array['vod_jumpurl'],$array['vod_pinyin']);
		...
		//播放列表解析
		$urllist = $this->ff_playlist_all($array);
	...
	* @返回$array 内容页重新组装的数组*/
	public function Lable_Vod_Play($array,$array_play,$createplayone){
		...
		$vod_play = explode('$$$', $array['vod_play']);
		$vod_url = explode('$$$', $array['vod_url']);
		$arrPlay = $arrUrl = [];
		...
		$array['vod_play'] = implode('$$$', $arrPlay);
		$array['vod_url'] = urlencode(implode('$$$', $arrUrl));
		$array['vod_server'] = implode('$$$', $arrServer);
		return $array;
	}
	//组合播放地址组列表为二维数组
	public function ff_playlist_all($array){
		...
		foreach($array_player as $sid=>$val){
			if (!in_array($val, ['down', 'bt', 'magnet'])) {
				...
				$playlist[$player[$val][0].'-'.$iii] = array('servername' => $array_server[$sid],'serverurl' => $server[$array_server[$sid]],'playername'=>$player[$val][1],'playname'=>$val,'son' => $this->ff_playlist_one($array_urllist[$sid],$array['vod_id'],$iii,$array['vod_cid'],$array['vod_name']));
				$iii++;
			} else {
				$downlist[$player[$val][0].'-'.$sid] = array('servername' => $array_server[$sid],'serverurl' => $server[$array_server[$sid]],'playername'=>$player[$val][1],'playname'=>$val,'son' => $this->ff_playlist_one($array_urllist[$sid],$array['vod_id'],$sid,$array['vod_cid'],$array['vod_name']));
			}
		}
		//ksort($playlist);
	    return ['play'=>$playlist, 'down'=>$downlist];
	}
	//分解单组播放地址链接
