...
* $jumpurl 跳转地址
*/
function ff_data_url($sid,$id,$cid,$name,$page,$jumpurl,$pinyin){
	//有跳转地址
	if ($jumpurl) {
		return $jumpurl;
	}
...
* $page 分页数字,大于1时返回的URL带有分页跳转参数变量{!page!}
*/
function ff_data_url_dir($sid,$id,$cid,$name,$page,$pinyin){
	//专题直接返回
	if('special' == $sid){
	...
	//影视或文章
	if('vod' == $sid){
		$datadir = str_replace_dir(C('url_voddata'),$id,$cid,$name,$pinyin);
	}else{
