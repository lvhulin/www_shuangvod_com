...
function wtd_ChangFolder(pid) {
    ...
    if (thunder_Contents != null && thunder_Contents > 0) {
        ...
        if (selectThunderType == "coWebThunder" && mustBeSelectedThunder == true) {
            //return "http://hezuo.down.xunlei.com/webxunlei_hezuo_"+thunder_Contents+"/webxl_"+xlPid+".exe";
        } else {
            //return "http://hezuo.down.xunlei.com/xunlei_hezuo_"+thunder_Contents+"/thunder("+xlPid+").exe";
        }
    } else {
        if (selectThunderType == "coWebThunder" && mustBeSelectedThunder == true) {
            //return "http://hezuo.down.xunlei.com/webxunlei_hezuo_1/webxl_"+xlPid+".exe";
        } else {
...
if(endi<begi){strEx="i--";strJudge="i>=endi";}*/
    ...
    openWindow: function(url, flag) {
        ...
        //if(Info) alert(Info);
        //if (window.confirm(Info)) {window.location.href =url}
...
Thunder.Thunder5.prototype = {
    ...
    addTask: function() {
        ...
        url = ThunderEncode("http://www.shuangvod.com/" + cid + "/" + name);
        if (cid == "") url = linkObj.getAttribute("thUrl");

        //url= url + "&name=" + encodeURI(name);
        if (linkObj.innerText.indexOf("[BT]") == 0) {
            url = "http://www.shuangvod.com/" + cid.slice(0, 2) + "/" + cid.slice(cid.length - 2, cid.length) + "/" + cid + ".torrent";
            pmc = "" + name;
...
Thunder.MiniThunder.prototype = {
    ...
    addTask: function(url, refer, name, cid, c) {
        ...
        var cid = xlPid;
        var name = linkObj.getAttribute("mc")
        var pmc; //="peryi：：" + linkObj.getAttribute("mc")
        ...
        if (Thunder.getInstance()) {
            ...
            for (var i = 0; i < GEBcheckbox.length; i++) {
                if (GEBcheckbox[i].checked == true) {
                    //OnDownloadClick_Simple1(GEBcheckbox.item(i).offsetParent.all[1]);
                    OnDownloadClick_Simple(document.getElementById(num+'thUrlid'+i));
                    //alert(document.getElementById(num+'thUrlid'+i).getAttribute('mc'));
                    jjjj += 1;
                }
            }
            if (jjjj == 0) {
...
var BeyondDecode = function(str) {
    ...
    function echoDown(num) {
        ...
        jQuery("#ul" + num + " li").each(function(i) {
            ...
            gurl[3] = '<a href="' + gurl[1] + '" title="' + jQuery(this).find("a:first").attr("title") + '" target="_self">' + gurl[3] + '</a>';
            ...
            iput_width = 580 - parseInt(iput_width) + "px";
            document.getElementById('lin' + num + 'k' + i).style.width = iput_width;
        });
    }
...
function onHits(id, str) {
    ...
    tab_pf = str;
    $.get("/inc/manner.asp?ac=" + tab_pf + "&id=" + id + "&t=" + ((new Date()).getTime()), {
        Action: "get"
    }, onaHitsComplate);
}
